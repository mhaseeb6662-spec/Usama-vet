    ...
    <div className="grid lg:grid-cols-12 gap-6">
      <div className="lg:col-span-8 space-y-4">
        ...
        {quote?.items.map((item) => (
          <FadeUp key={item.productId} className="bg-white border border-slate-200 rounded-xl p-4 flex gap-4">
            ...
            <div className="flex-1 min-w-0">
              ...
              <div className="mt-3 flex items-center gap-3">
                <div className="flex items-center border border-slate-200 rounded-lg">
                  <button type="button" onClick={() => setQuantity(item.productId, item.quantity - 1, item.stock)} className="w-8 h-8 flex items-center justify-center" aria-label="Decrease quantity">
                    <Minus className="w-3.5 h-3.5" />
                  </button>
                  <span className="w-8 text-center text-sm font-semibold">{item.quantity}</span>
                  <button type="button" onClick={() => setQuantity(item.productId, item.quantity + 1, item.stock)} className="w-8 h-8 flex items-center justify-center" aria-label="Increase quantity">
                    <Plus className="w-3.5 h-3.5" />
                  </button>
                </div>
                <button type="button" onClick={() => removeItem(item.productId)} className="text-rose-500 text-sm flex items-center gap-1">
                  <Trash2 className="w-4 h-4" /> Remove
                </button>
              </div>
            </div>
            <div className="font-semibold text-slate-900 shrink-0">Rs. {(item.unitPrice * item.quantity).toLocaleString()}</div>
          </FadeUp>
        ))}
      </div>
      <aside className="lg:col-span-4">
        <div className="bg-white border border-slate-200 rounded-xl p-5 space-y-3 sticky top-28">
          ...
          {quote?.canCheckout ? (
            <Link href="/checkout" className="block text-center bg-[#009473] hover:bg-[#028467] text-white font-semibold py-3 rounded-lg">
              Proceed to Checkout
            </Link>
          ) : (
