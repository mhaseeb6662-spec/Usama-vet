      ...
      <div className="bg-gradient-to-br from-emerald-900 to-emerald-950 text-white rounded-2xl p-8 md:p-12 text-center shadow-xl relative overflow-hidden">
        ...
        <div className="relative z-10 max-w-2xl mx-auto space-y-6">
          ...
          <div className="space-y-2">
            <h3 className="text-xl sm:text-2xl md:text-3xl font-extrabold tracking-wide font-sans">
              تلاش کریں اپنی پسندیدہ پروڈکٹس
            </h3>
            <p className="text-xs sm:text-sm text-emerald-200 font-light max-w-lg mx-auto">
          ...
          <div className="w-full flex max-w-xl mx-auto shadow-md rounded-lg overflow-hidden">
            <input
              ...
              className="w-full bg-white text-slate-800 text-xs sm:text-sm px-4 py-3 focus:outline-none placeholder:text-slate-400"
              disabled
            />
            <button
              type="button"
              className="bg-emerald-600 hover:bg-emerald-700 text-white font-bold text-xs uppercase px-6 flex items-center gap-1.5 transition-colors cursor-not-allowed shrink-0"
              disabled
            >
          ...
          <div className="pt-2">
            <span className="text-[10px] sm:text-xs text-emerald-300 font-bold uppercase tracking-wider block mb-2.5">
              Popular Searches:
            </span>
            <div className="flex flex-wrap justify-center gap-2">
              {popularSearches.map((chip) => (
                <span
                  key={chip}
                  className="bg-emerald-800/50 hover:bg-emerald-800 border border-emerald-700/60 text-emerald-100 text-[10px] sm:text-xs px-3 py-1.5 rounded-full cursor-not-allowed font-medium transition-colors"
                >
                  {chip}
                </span>
              ))}
