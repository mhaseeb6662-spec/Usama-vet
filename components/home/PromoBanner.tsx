      ...
      <FadeUp distance={14} duration={0.45}>
        <div className={`relative rounded-xl overflow-hidden shadow-sm p-6 sm:p-8 flex flex-col sm:flex-row justify-between items-center gap-4 text-center sm:text-left ${bgClass} ${textClass}`}>
          ...
          <div className="relative z-10 shrink-0">
            <Link
              href={href}
              className="bg-white hover:bg-slate-50 text-slate-900 hover:text-emerald-700 font-semibold text-xs sm:text-[13px] uppercase px-5 py-2.5 sm:py-3 rounded-lg shadow-md transition-all hover-scale-button flex items-center gap-1.5 focus:outline-none focus:ring-2 focus:ring-emerald-500 focus:ring-offset-2 group cursor-pointer"
            >
              <span>{btnText}</span>
              <ArrowRight className="w-4 h-4 text-emerald-600 shrink-0 transition-transform duration-200 group-hover:translate-x-[3px]" />
            </Link>
          </div>
        </div>
      </FadeUp>
